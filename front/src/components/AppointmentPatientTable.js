...
import '../css/appointmentTablePatient.css';
import MaterialTable from 'material-table'
import { appointmentsRoute } from '../axios/routes';
import { deleteAppointment, updateAppointmentPatient, postAppointment } from '../axios/controllers';
import Select from "react-select";

const TabelProgramari = ({ programari, id, fisaMed }) => {
  const dates = [
  ...
  const columns = [
    ...
    // { title: "Tratament", field: "treatment", editable: false },
    // { title: "Recomandari", field: "recommendations", editable: false },
    // { title: "Status", field: "status" },
  ];

  return (
    <div className="appointmentTablePatient">
      <MaterialTable
        title="Programari"
        data={programari}
        ...
        editable={{
          onRowAdd: (newRow) => new Promise((resolve, reject) => {
            try {
              postAppointment(appointmentsRoute, { newRow, id });
            } catch (error) {
              console.log(error);
            }
          ...
          onRowUpdate: (updatedRow, oldRow) => new Promise((resolve, reject) => {
            try {
              console.log(oldRow);
              updateAppointmentPatient(appointmentsRoute, { date: updatedRow.date, symptoms: updatedRow.symptoms, other_information: updatedRow.other_information, doctorId: updatedRow.doctorId }, oldRow.appointmentId);
            }
            catch (err) {
              console.log(err);
            }
